...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$title = $_POST['title'];
	$stmt = $pdo->prepare("SELECT * FROM draftquestions WHERE title = :title");
    ...
    foreach ($questionTypes as $key => $questionType) {
        ...
        $stmt = $pdo->prepare("INSERT INTO draftquestions (title,question_type, question) VALUES (?, ?, ?)");
        ...
        if ($questionType === 'radio' || $questionType === 'checkbox') {
            ...
            foreach ($options as $option) {
                $stmt = $pdo->prepare("INSERT INTO draftoptions (question_id, title, option_text) VALUES (?, ?, ?)");
                $stmt->execute([$questionId, $title, $option]);
            }
        }
    }

    echo "Survey drafted successfully";
	}
	else {
